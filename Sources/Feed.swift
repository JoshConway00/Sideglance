...
@MainActor final class Feed: ObservableObject {
    ...
    func title(for entry: Entry) -> String? {
        ...
        return titles[key] ?? directTitles[key]
    }
    ...
    func labelOrigin(for entry: Entry) -> String {
        ...
        return directTitles[key] == nil ? "original_text_fallback" : "local_excerpt_no_model"
    }
    ...
    func dismiss(_ entry: Entry) { dismissed.insert(entry.id) }
    ...
    var visible: [Entry] {
        let filtered = activityBodyEntries(notificationCards, includeTools: showActivity).filter { !dismissed.contains($0.id) }.map { entry in
            var card = entry
            if !showActivity { card.liveStatus = nil }
            return card
        }
        let candidates = groupErrors ? groupedErrors(filtered) : filtered
        let pinnedID = keepLatest && status == .working ? candidates.last?.id : nil
        return Array(candidates.filter { editing || $0.id == pinnedID || isRecent($0, now: now, lifetime: displayLifetime($0, base: lifetime, adaptive: adaptiveTiming)) }.suffix(2))
    }
    func start() {
        poll()
        timer?.invalidate()
                ...
                for candidate in update.messages {
                    if var entry = revisions.accept(candidate, receivedAt: loaded ? Date() : nil) {
                        ...
                        if narrativeBatch.add(entry, now: Date()) {
                            ...
                            directTitles[entry.id] = compactActivityText(entry.text, limit: 96)
                            QualityLog.shared.record("label_decision", ["task_id": task.id, "entry_id": entry.id, "source_key": entry.id, "strategy": "local_excerpt_until_multiple_narrative_updates", "display_label": directTitles[entry.id] ?? "", "model_used": "none"])
                        ...
                        if [.progress, .activity, .error, .summary].contains(entry.kind) { entry.intentionKey = intentionKey }

...
func activityBodyEntries(_ entries: [Entry], includeTools: Bool) -> [Entry] {
    entries.filter { $0.kind == .summary || $0.kind == .error || (includeTools && $0.kind == .activity) }
}
...
func decode(_ line: Data) -> Decoded {
    ...
    case "McpToolCall":
        ...
        text = (item["actionName"] as? String ?? item["tool"] as? String ?? "Tool") + " · " + (item["status"] as? String ?? "completed")

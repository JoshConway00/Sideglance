...
@MainActor final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let duration = submenu("Disappear after", in: updates)
        for seconds in [10, 20, 30, 60, 120] {
            let option = action("\(seconds) seconds", #selector(setLifetime(_:)), in: duration, checked: Int(feed.lifetime) == seconds)
            option.tag = seconds
        }
        _ = action("Keep latest while working", #selector(toggleKeepLatest(_:)), in: updates, checked: feed.keepLatest)
        _ = action("Adaptive reading time", #selector(toggleAdaptive(_:)), in: updates, checked: feed.adaptiveTiming)

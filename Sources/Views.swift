    ...
    var body: some View {
        HStack(alignment: .top, spacing: 8) {
            VStack(alignment: .leading, spacing: 7) {
                if !activity.isEmpty { HStack(spacing: 6) {
                    ...
                    Text("Codex · " + notificationPlainText(activity))
                        ...
                        .foregroundStyle(.white.opacity(0.55)).fixedSize(horizontal: false, vertical: true)
                }
                }
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if !subtitle.isEmpty {
                        Text(notificationPlainText(subtitle)).font(.system(size: fontSize - 1, weight: .medium))
                            .foregroundStyle(.white.opacity(0.66)).fixedSize(horizontal: false, vertical: true)
                    }
                }
...
struct Overlay: View {
    ...
    private func recordPresentation(_ entry: Entry) {
        ...
        QualityLog.shared.record("notification_presented", [
            ...
            "display_status": "Codex · " + feed.currentActivityLabel,
            ...
            "display_transform": "Markdown to plain text; collapse whitespace; status 1 line; heading and summary wrap without truncation",
    ...
    var body: some View {
        ...
        VStack(alignment: .trailing, spacing: 10) {
            ...
            if let entry = visible.last {
                NotificationCard(activity: feed.currentActivityLabel,
                    ...
                    isError: feed.currentActivityNeedsAttention, fontSize: feed.fontSize, opacity: feed.opacity,
                    dismiss: { feed.dismiss(entry) }, open: openTask)
                    .onAppear { recordPresentation(entry) }
                    .onChange(of: [feed.promptTitle(for: entry) ?? "", feed.title(for: entry) ?? "", entry.text, entry.liveStatus?.entryID ?? "", feed.currentActivityLabel, feed.task.title, String(feed.fontSize)]) { recordPresentation(entry) }
                    .transition(.opacity.combined(with: .move(edge: .top)))
            }
            if feed.intentionTitles, let notice = feed.summaryAvailabilityNotice {
                Text("Summaries paused · " + notice)

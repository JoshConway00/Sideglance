...
func runRegressionTests() {
    ...
    let outcome = "Tests passed for error → progress, new command, and completion transitions."
    assert(cleanIntentionTitle(outcome, isPrompt: false) == outcome)
    assert(cleanIntentionTitle(outcome, isPrompt: true) == nil)
    assert(cleanIntentionTitle(String(repeating: "word ", count: 30), isPrompt: false) == nil)
    assert(cleanIntentionTitle("Selecting MIT and checking GitHub sign-in") != nil)
    assert(cleanIntentionTitle("Selecting MIT and...") == nil)
    assert(cleanIntentionTitle("Selecting MIT and…") == nil)
    assert(!compactActivityText(String(repeating: "long ", count: 50)).contains("…"))
    ...
    assert(activityBodyEntries(bodyFixture, includeTools: true).map(\.id) == ["p", "s", "t", "a"])
    assert(activityBodyEntries(bodyFixture, includeTools: false).map(\.id) == ["p", "s", "a"])
    ...
    let namedStart: [String: Any] = ["type": "response_item", "payload": ["type": "function_call", "name": "js", "namespace": "mcp__cua_repl", "arguments": "{\"title\":\"Verify the final page\",\"code\":\"private code\"}", "call_id": "named"]]
    let start = decode(try! JSONSerialization.data(withJSONObject: namedStart)).messages.first!
    assert(start.text == "Verify the final page")
    assert(toolDisplayTitle(["title": "Document title"], tool: "create_document", server: "other") == nil)
    let namedEnd: [String: Any] = ["type": "event_msg", "payload": ["type": "item_completed", "item": ["type": "McpToolCall", "server": "cua_repl", "tool": "js", "arguments": ["title": "Verify the final page", "code": "private code"], "status": "completed"]]]
    let end = decode(try! JSONSerialization.data(withJSONObject: namedEnd)).messages.first!
    assert(liveActivityLabel(end) == "Verify the final page · completed")
...
@MainActor func runStateRegressionTests() {
    ...
    feed.paused = true; feed.intentionTitles = false; feed.showActivity = true; feed.inputSound = false
...
func runNarrativeBatchTests() {
    ...
    assert(batch.ready(now: now.addingTimeInterval(2))?.entryIDs == ["first"])
    ...
    assert(batch.ready(now: now.addingTimeInterval(1)) == nil)
    assert(batch.ready(now: now.addingTimeInterval(2))?.entryIDs == ["second"])
    assert(batch.ready(now: now.addingTimeInterval(2))?.source.contains("Adjusting notification padding") == true)
    assert(batch.ready(now: now.addingTimeInterval(2))?.source.contains("Checking notification spacing") == false)
    batch.consume()
    assert(batch.ready(now: now.addingTimeInterval(10), force: true) == nil)
    assert(!batch.add(first, now: now))
    assert(batch.add(entry("third", .progress, "Checking notification controls"), now: now))
    assert(batch.ready(now: now.addingTimeInterval(2))?.entryIDs == ["third"])
    ...
    assert(group.entryIDs == ["long7"] && group.source.count < 2500)
...
@MainActor func runNotificationUpdateTests() {
    ...
    feed.paused = true; feed.intentionTitles = false; feed.showActivity = true
    ...
    assert(feed.visible.count == 1 && feed.visible.last?.id == feed.entries.last?.id)
    assert(feed.visible.allSatisfy { ($0.liveStatus?.kind ?? $0.kind) == .error })
    apply([Decoded(entry: entry("recovery-progress", .progress, "Checking the successful retry"))])
    assert(feed.visible.allSatisfy { liveActivityLabel($0) == "Checking the successful retry" })
    assert(feed.visible.allSatisfy { ($0.liveStatus?.kind ?? $0.kind) != .error })
    assert(feed.entries.contains { $0.kind == .error })
    apply([Decoded(entry: entry("retry-running", .activity, "Running command"))])
    assert(feed.visible.allSatisfy { liveActivityLabel($0) == "Running command" })
    apply([Decoded(entry: entry("retry-error", .error, "Command failed"))])
    apply([Decoded(state: .finished)])
    assert(feed.visible.allSatisfy { liveActivityLabel($0) == "Finished" && $0.liveStatus?.kind == .activity })
    ...
    let stableFeed = Feed()
    stableFeed.status = .working
    stableFeed.intentionTitles = true
    let old = Entry(id: "retained", kind: .summary, text: "The build passed", time: stamp, intentionKey: "ready", promptKey: "request")
    stableFeed.appendNotification(old)
    stableFeed.titles["ready"] = "Build passed; deployment remains pending"
    let next = Entry(id: "pending", kind: .progress, text: "Checking release settings", time: stamp, intentionKey: "pending", promptKey: "request")
    stableFeed.appendNotification(next)
    assert(stableFeed.visible.map(\.id) == ["retained"])
    stableFeed.titles["pending"] = "Checking release settings"
    assert(stableFeed.visible.last?.id == "pending")
    stableFeed.appendNotification(entry("failure", .error, "Command failed"))
    assert(stableFeed.currentActivityNeedsAttention)
    stableFeed.appendNotification(entry("recovery", .activity, "Running command"))
    assert(!stableFeed.currentActivityNeedsAttention && !stableFeed.visible.contains { $0.kind == .error })
    stableFeed.now = Date().addingTimeInterval(86400)
    stableFeed.status = .finished
    assert(stableFeed.visible.count == 1)
    stableFeed.dismiss(stableFeed.visible[0])
    assert(stableFeed.visible.isEmpty)
    stableFeed.status = .working
    stableFeed.appendNotification(entry("named-activity", .summary, "Building income source summary"))
    assert(stableFeed.currentActivityLabel == "Building income source summary")
    stableFeed.appendNotification(entry("long-progress", .progress, String(repeating: "Long progress text ", count: 20)))
    assert(stableFeed.currentActivityLabel == "Working")
    stableFeed.inputRequested = true
    assert(stableFeed.currentActivityNeedsAttention && stableFeed.currentActivityLabel == "Input requested")
